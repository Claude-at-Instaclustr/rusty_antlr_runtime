...
/// parse tree node.
pub trait RecogniserNodeType<'input>: TidAble<'input> + Sized {
    /// Shortcut for `Type::TF`
    type TF: TokenFactory<'input> + 'input;
    /// Actual type of the parse tree node
    type Type: ?Sized + RuleContext<'input, Ctx = Self, TF = Self::TF> + 'input;
...
pub trait Recognizer<'input>: TokenAware<'input> {
    ...
    fn sempred(&mut self, rule_index: isize, action_index: isize) -> bool
    where
        Self: Sized,
    {
        self.get_node().sempred(rule_index, action_index)
    }

    fn action(&mut self, rule_index: isize, action_index: isize)
    where
        Self: Sized,
    {
        self.get_node().action(rule_index, action_index);
    }
...
pub trait Actions<'a, P: Recognizer<'a>> {
    fn sempred(
        _localctx: Option<&<P::Node as RecogniserNodeType<'a>>::Type>,
        _rule_index: isize,
        _action_index: isize,
        _recog: &mut P,
    ) -> bool {
